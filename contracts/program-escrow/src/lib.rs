...
//! Γöé              Program Escrow Architecture                         Γöé
...
//! Γöé  Γöé  Organizer   Γöé                                               Γöé
//! Γöé  ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö¼ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ                                               Γöé
//! Γöé         Γöé                                                        Γöé
//! Γöé         Γöé 1. init_program()                                     Γöé
//! Γöé         Γû╝                                                        Γöé
//! Γöé  ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ                                           Γöé
//! Γöé  Γöé  Program Created Γöé                                           Γöé
//! Γöé  ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö¼ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ                                           Γöé
//! Γöé           Γöé                                                      Γöé
//! Γöé           Γöé 2. lock_program_funds()                             Γöé
//! Γöé           Γû╝                                                      Γöé
//! Γöé  ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ                                           Γöé
//! Γöé  Γöé  Funds Locked    Γöé                                           Γöé
//! Γöé  Γöé  (Prize Pool)    Γöé                                           Γöé
//! Γöé  ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö¼ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ                                           Γöé
//! Γöé           Γöé                                                      Γöé
//! Γöé           Γöé 3. Hackathon happens...                             Γöé
//! Γöé           Γöé                                                      Γöé
//! Γöé  ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓû╝ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ                                           Γöé
//! Γöé  Γöé Authorized       Γöé                                           Γöé
//! Γöé  Γöé Payout Key       Γöé                                           Γöé
//! Γöé  ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö¼ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ                                           Γöé
//! Γöé           Γöé                                                      Γöé
//! Γöé    ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓö┤ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ                                             Γöé
//! Γöé    Γöé              Γöé                                             Γöé
//! Γöé    Γû╝              Γû╝                                             Γöé
//! Γöé batch_payout() single_payout()                                  Γöé
//! Γöé    Γöé              Γöé                                             Γöé
//! Γöé    Γû╝              Γû╝                                             Γöé
//! Γöé ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ                                    Γöé
//! Γöé Γöé   Winner 1, 2, 3, ...   Γöé                                    Γöé
//! Γöé ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ                                    Γöé
//! Γöé                                                                  Γöé
//! Γöé  Storage:                                                        Γöé
...
//! Γöé  Γöé ProgramData:                             Γöé                  Γöé
//! Γöé  Γöé  - program_id                            Γöé                  Γöé
//! Γöé  Γöé  - total_funds                           Γöé                  Γöé
//! Γöé  Γöé  - remaining_balance                     Γöé                  Γöé
//! Γöé  Γöé  - authorized_payout_key                 Γöé                  Γöé
//! Γöé  Γöé  - payout_history: [PayoutRecord]        Γöé                  Γöé
//! Γöé  Γöé  - token_address                         Γöé                  Γöé
...
// ΓöÇΓöÇ Step 1: Add module declarations near the top of lib.rs ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
/// Transitions:
/// ```text
/// (none) ΓöÇΓöÇopen_dispute()ΓöÇΓöÇΓû║ Open ΓöÇΓöÇresolve_dispute()ΓöÇΓöÇΓû║ Resolved
/// ```
#[contracttype]
#[derive(Clone, Debug, Eq, PartialEq)]
...
///
/// ### Fields
/// - `window_size`  ΓÇô Rolling window duration in seconds (must be > 0).
/// - `max_amount`   ΓÇô Maximum total amount releasable within one window.
/// - `enabled`      ΓÇô When `false` the config is persisted but not enforced.
///
/// ### Upgrade safety
...
///
/// ### Fields
/// - `window_start`     ΓÇô Ledger timestamp of the current window's start.
/// - `amount_released`  ΓÇô Cumulative amount released within the current window.
///
/// ### Atomicity guarantee
...
pub enum DataKey {
    ...
    ReentrancyGuard,
    /// Idempotency key record ΓÇö keyed by the caller-supplied string key.
...
/// V2 audit event for pause state changes ΓÇö deterministic, upgrade-safe.
///
...
///   event accurately reflects the transition (old ΓåÆ new).
...
impl ProgramEscrowContract {
    ...
    pub fn initialize_program(
        ...
        // ΓöÇΓöÇ Token allowlist enforcement ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    pub fn lock_program_funds(env: Env, amount: i128) -> ProgramData {
        ...
        // Store updated data ΓÇö sync both legacy PROGRAM_DATA and keyed program storage
    ...
    /// version marker ΓÇö callers should treat `0` as "unknown / pre-v1".
    ...
    /// circuit breaking. Idempotent ΓÇö safe to call multiple times.
    ...
    /// Alias for get_analytics ΓÇö used by some test modules.
    ...
    ///
    /// # Arguments
    /// * `program_id`      ΓÇö The program whose payout key should be rotated.
    /// * `caller`          ΓÇö The address initiating the rotation (must be current
    ///                       payout key or contract admin); their auth is required.
    /// * `new_key`         ΓÇö The replacement payout key (must differ from current).
    /// * `expected_nonce`  ΓÇö Must equal the current stored rotation nonce;
    ///                       prevents replaying a prior signed rotation request.
    ///
    /// # Panics
    /// * `"New key must differ from current key"` ΓÇö self-rotation attempt.
    /// * `"Invalid nonce"` ΓÇö `expected_nonce` does not match the stored nonce.
    /// * `"Unauthorized"` ΓÇö caller is neither the current payout key nor admin.
    ...
    fn enforce_token_allowlist(env: &Env, token_address: &Address, program_id: &String) {
        ...
        if allowlist.is_empty() {
            // Allowlist is empty ΓåÆ enforcement disabled, accept any token.
            return;
        }
        ...
        // Token not found ΓÇö emit rejection event then panic.
        env.events().publish(
            (TOKEN_REJECTED,),
            TokenRejectedEvent {
                version: EVENT_VERSION_V2,
    ...
    /// is disabled ΓÇö all tokens are accepted again.
    ///
    /// # Errors
    ...
    pub fn remove_allowed_token(env: Env, token: Address) {
        ...
        for existing in allowlist.iter() {
            if existing == token {
                found = true;
                // Skip ΓÇö effectively removes it.
            } else {
    ...
    ///
    /// This is a pure view ΓÇö no auth required.
    ...
    fn batch_payout_internal(
        ...
        // 8.  Pre-validate fees for every entry (atomicity ΓÇö no partial state)
        // 9.  Execute transfers
        ...
        // 3c. Circuit breaker ΓÇö single authoritative check before all business
        ...
        // 9. Execute transfers ΓÇö all pre-validation passed; this section must not fail.
    ...
    fn single_payout_internal(
        ...
        // 3. Paused (operational state)
        // 3b. Dispute guard
        // 3c. Circuit breaker ΓÇö before all business logic for deterministic rejection
        // 4. Authorization
        ...
        // 1b. Idempotency check ΓÇö runs before any state reads so duplicate
        ...
        // 3b. Dispute guard ΓÇö payouts blocked while a dispute is open
        if Self::dispute_state(&env) == DisputeState::Open {
            panic!("Payout blocked: dispute open");
        }

        // 3c. Circuit breaker check ΓÇö runs before all business logic so that
    ...
    fn trigger_program_releases_internal(env: Env, caller: Option<Address>) -> u32 {
        ...
        for k in 0..due_indices.len() {
            ...
            // Skip schedule if contract has insufficient balance ΓÇö deferred to next trigger
            if schedule.amount > program_data.remaining_balance {
                skipped_count += 1;
                continue;
            }
